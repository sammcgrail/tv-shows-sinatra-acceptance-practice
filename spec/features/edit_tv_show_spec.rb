require "spec_helper"

feature "user edits an existing TV show" do
  # As a TV fanatic
  # I want to edit an existing show
  # So that I can correct any foolish mistakes
  #
  # Acceptance Criteria:
  # * Editing a show provides a pre-populated form for each field.
  # * Submitting the edit form will update a show if all validations pass.
  # * The user is redirected to the details page
  # * for that show if successfully updated.
  # * If the update fails any validations, re-display
  # * the form with the appropriate error messages.


  scenario "when user clicks edit on a show's info page, they are brought to an edit form" do
  

  end
end
